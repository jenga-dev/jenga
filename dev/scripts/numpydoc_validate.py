...
NAME_SKIP = [
    ...
    "Union",
    "Iterable",
    # jenga types that are not objects that should be checked
    # jenga globals we can skip
]
...
def recursively_validate_object(val_obj, val_full_name, val_name) -> List[str]:
...
def validate_module(module_name: str) -> bool:
    ...
    obj_w_hard_errors = []
    rep_res = get_npdoc_val_report(module_name)
    if rep_res:
    ...
    for name, obj in inspect.getmembers(sys.modules[module_name]):
        ...
        if inspect.ismodule(obj) and name not in module_blacklist:
            ...
            # print(f"Found jenga sub-module {module_full_name}")
...
if __name__ == "__main__":
    errors_found = validate_module("jenga")
    if errors_found:
        sys.exit(1)
